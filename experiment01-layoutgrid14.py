...
def update_font_size():
    text_field.config(font=("TkDefaultFont", 14))
    for row in range(3):
        for col in range(3):
            if entries[row][col] is not None:
                entries[row][col].config(font=("TkDefaultFont", 14))
...
def open_template_maker():
    ...
    label.pack(pady=20)
    text_field = tk.Text(template_window, wrap='word', height=10, width=40, font=("TkDefaultFont", 14))
...
for row in range(3):
    for col in range(3):
        if row == 0 and col == 0:
            ...
            import_btn = tk.Button(frame, text="Import JSON", command=import_json, font=("TkDefaultFont", 14))
            import_btn.pack(side=tk.TOP, fill=tk.X)
            export_btn = tk.Button(frame, text="Export JSON", command=export_json, font=("TkDefaultFont", 14))
