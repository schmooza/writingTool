    ...
    for row in range(3):
        for col in range(3):
            if entries[row][col] is not None:
                ...
                # Use regex to split text into words while ignoring punctuation
                ...
                for word in misspelled:
                    start_idx = "1.0"
                    while True:
                        # Use regex to search for whole words to avoid partial matches
                        start_idx = text_widget.search(r'\b' + re.escape(word) + r'\b', start_idx, stopindex=tk.END, nocase=True, regexp=True)
                        if not start_idx:
                            break
